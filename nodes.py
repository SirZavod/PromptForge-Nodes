...
1. PASSTHROUGH NODE  (PromptForgeConnection)
   Five inputs (prompt/seed/width/height/negative_prompt) → five outputs of the same types.
   negative_prompt is optional — not every model/workflow uses a negative
   prompt, so the node doesn't force a negative CLIPTextEncode connection.
   PromptForge finds this node by class_type and patches the five values
   before submitting the workflow to /prompt.
...
class PromptForgeConnection:
    """Pure passthrough: 5 inputs in, the same 5 values out.

    PromptForge locates this node by class_type in the graph it fetched
    from /promptforge/graph, patches the five inputs, then submits the
    whole graph to ComfyUI's /prompt. The node itself just forwards the
    values unchanged so they can be wired wherever needed.
    """
    ...
    RETURN_NAMES  = ("prompt", "seed", "width", "height", "negative_prompt")
    ...
    DESCRIPTION   = (
        ...
        "don't use a negative prompt. "
        "Works as a normal node for manual use too."
    )

    def passthrough(self, prompt, seed, width, height, negative_prompt=""):
        return (prompt, seed, width, height, negative_prompt)
